...
def is_student(user):
    return (
        user.is_authenticated
        and hasattr(user, "profile")
        and user.profile.user_type == "student"
    )
...
def dashboard(request):
    if not hasattr(request.user, "profile"):
        ...
        UserProfile.objects.create(user=request.user, user_type=user_type)
...
def profile(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # Update the User model's first_name and last_name
        ...
        if form.is_valid():
            ...
            for file in files:
                FeedbackAttachment.objects.create(
                    feedback=feedback, file=file, filename=file.name
                )
...
def feedback_detail(request, feedback_id):
    ...
    if not (
        request.user == feedback.student
        or request.user == feedback.teacher
...
def download_attachment(request, attachment_id):
    ...
    if not (
        request.user == attachment.feedback.student
        or request.user == attachment.feedback.teacher
        or is_admin(request.user)
    ):
    ...
    if request.method == "POST":
        if "user_id" in request.POST and "new_role" in request.POST:
            ...
            try:
                ...
                messages.success(
                    request,
                    f"Роль пользователя {profile.user.username} изменена на {profile.get_user_type_display()}",
                )
...
def manage_university_info(request):
    ...
    return render(
        request, "main/manage_university_info.html", {"info_pages": info_pages}
    )
...
def mark_public_feedback_worked(request, feedback_id):
    if request.method == "POST":
        try:
            ...
            feedback.save()
            messages.success(
                request, f"Обращение от {feedback.name} отмечено как обработанное"
            )
...
def download_teacher_statistics(request):
    ...
    for teacher_profile in teachers:
        ...
        worksheet.write(
            row, 0, teacher.get_full_name() or teacher.username, cell_format
        )
